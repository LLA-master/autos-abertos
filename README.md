...
- `pipeline/` — os scripts que produzem esses dados a partir do acervo bruto (que não está neste repositório e não deve estar).
  `stage7_wiki.py` monta as fichas dos personagens (`docs/data/wiki.json`, `wiki/*.md`) a partir dos dados públicos e da curadoria em `pipeline/personagens.py`: condição nos autos (investigado ou alvo de medida, autoridade, advogado, instituição oficiada, citado), biografia e âmbito por processo, em PT e EN, cada afirmação com processo, peça e página; conta também em quantos atos do juízo publicados cada nome aparece. Quem está em `pipeline/protegidos.py` (vítimas, testemunhas, familiares) é pseudonimizado pelo estágio 6 e nunca ganha ficha.
  ...
  `stage11_decisoes.py` publica o texto integral dos atos decisórios (decisões monocráticas, despachos e acórdãos; certidões e manifestações da PGR ficam fora desde 17.09.2026) com os dados pessoais mascarados (curadoria em `pipeline/protegidos.py`). O site os lista na seção Decisões e os abre a partir das páginas de processo.
