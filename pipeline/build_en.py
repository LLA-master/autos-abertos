...
"""Gera a versão em inglês do site a partir dos originais em português.
  ...
  pipeline/en/primer.html (Brazil primer) inserido em en.html; as biografias (PT e EN) vivem em pipeline/personagens.py
  ...
  docs/primeyou.en.html      (edição em inglês do dossiê; é FONTE, escrita à mão, como pipeline/en/primer.html)
...
def wiki_en(w):
    """wiki.json → wiki_en.json: os campos curados têm versão em inglês (sub_en, bio_en, amb_en) escrita em
    pipeline/personagens.py; o resumo automático é regerado em inglês; as definições de condição e os
    títulos das seções vêm de t_en/d_en. Os campos *_en não sobrevivem na saída."""
    meta = {"cond": {k: {"t": v["t_en"], "s": v["s_en"], "d": v["d_en"]} for k, v in w["meta"]["cond"].items()},
            "grupos": [{"g": g["g"], "cond": g["cond"], "t": g["t_en"]} for g in w["meta"]["grupos"]]}
    out = {"gerado_em": w["gerado_em"], "meta": meta, "pages": []}
    for p in w["pages"]:
        q = dict(p)
        if p.get("resumo"):
            ...
            s = (f"{p['label']} appears in {p['docs']} narrative filings across {p['procs']} of the 15 proceedings, "
                 f"most often in {proc_txt}. Automatic classification: {ROLE_EN.get(p['papel'], p['papel'])}.")
            if p.get("peak"): s += f" The dates cited on the pages where the name appears cluster around {p['peak']}."
            q["resumo"] = s
        for f in ("sub", "bio", "amb"):
            if f + "_en" in q: q[f] = q.pop(f + "_en")
        out["pages"].append(q)
    return out
...
def main():
    ...
    secs = (ROOT/"pipeline"/"en"/"primer.html").read_text(encoding="utf-8")   # o primer entra antes dos processos; as biografias vivem em personagens.py
    # {{sel:Rótulo}} → id do personagem (link para a ficha); falha se o rótulo não existir na base
    w = json.load(open(DOCS/"data"/"wiki.json", encoding="utf-8"))
    g = {"nodes": json.load(open(DOCS/"data"/"nodes.json", encoding="utf-8"))}
    ids = {n["label"]: n["id"] for n in g["nodes"] if n.get("vis")}
    def sel(m):
        lab = m.group(1)
        if lab not in ids: sys.exit(f"BUILD EN FALHOU: rótulo sem personagem visível na base: {lab!r}")
        return ids[lab]
    ...
    assert html.count('<section id="v-processos"') == 1 and html.count('<div id="crCards"></div>') == 1
    html = html.replace('<section id="v-processos"', secs.rstrip()+"\n\n"+'<section id="v-processos"', 1)   # o primer entra antes dos processos
