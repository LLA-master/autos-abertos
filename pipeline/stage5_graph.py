...
if splits:
    ...
    c.execute("INSERT INTO names SELECT s.a, n.doc_id, n.processo, n.page, n.pos, n.tipo, n.empresa FROM names n JOIN splits s USING(value)")
    c.execute("INSERT INTO names SELECT s.b, n.doc_id, n.processo, n.page, n.pos, n.tipo, n.empresa FROM names n JOIN splits s USING(value)")
...
canon={}; longs=[]
def tokens(v): return [t for t in v.split() if t.upper() not in ("DE","DA","DO","DAS","DOS","E","DI","DEL")]
def subseq(s,l):
...
c.execute(f"""CREATE TABLE allm AS SELECT c.canon entity, m.doc_id, m.page, d.tipo, d.pages npages,
   ...
   FROM m JOIN d ON d.id=m.doc_id JOIN canon c ON c.value=trim(regexp_replace(m.value,'\\s+(LTDA|EIRELI|ME|EPP|S\\.?A\\.?|S/A|DTVM|CCTVM|CIA|LTD|LLC|INC)\\.?$',''))
