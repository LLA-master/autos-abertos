...
function renderWiki(){ const grid=$("#wkGrid"), pg=$("#wkPage");
  if(WK.open&&wkBy.has(WK.open)){ const p=wkBy.get(WK.open); grid.hidden=true; pg.hidden=false;
    ...
    pg.innerHTML=`<button class="btn ghost small" id="wkBack">← all profiles</button><h2 style="margin-top:12px">${p.label}</h2><span class="badge" style="--c:var(--${p.papel})">${ROLE_LABEL[p.papel]}</span>
      ...
      <p>${p.resumo}</p>
      <div class="acts"><button class="btn small" data-open="${p.id}">Open in the graph</button><button class="btn small" data-mmc="${p.id}">Mind map</button><a class="btn ghost small" href="https://github.com/LLA-master/autos-abertos/blob/main/wiki/${p.slug}.md" target="_blank" rel="noopener">Profile in Markdown (Portuguese)</a></div>
...
async function renderCronicas(qs){ const posts=await loadCR(); const p=new URLSearchParams(qs||""); const slug=p.get("p"); const L=$("#crList"), P=$("#crPost");
  if(slug){ const i=posts.findIndex(x=>x.slug===slug); if(i>=0){ const post=posts[i]; L.hidden=true; P.hidden=false; P.innerHTML=`<p class="muted">Loading…</p>`;
      let md=""; try{ const r=await fetch(`posts/${post.slug}.md`); if(!r.ok) throw new Error(r.status); md=stripFM(await r.text()); }catch(e){ md="*The text of this chronicle could not be loaded.* Try reloading the page; if it persists, open an issue in the repository."; }
      ...
      const eyebrow=post.serie?`${esc(EN_SERIE(post.serie))} · Chapter ${post.capitulo} of ${inS.length} · ${dateBR(post.date)} · ${post.minutes} min · in Portuguese`:`Chronicle ${String(post.numero||posts.length-i).padStart(2,"0")} · ${dateBR(post.date)} · ${post.minutes} min read · in Portuguese`;
      P.innerHTML=`<button class="btn ghost small back" id="crBack">← all chronicles</button><p class="eyebrow">${eyebrow}</p><h1>${esc(EN_T(post))}</h1><p class="sub">${esc(EN_S(post))}</p>
        <div class="meta">${(post.tags||[]).map(t=>`<span class="tag">${esc(t)}</span>`).join("")}<span class="muted">The project author's opinion, published in Portuguese. Numbers and citations point to the public data and to the STF archive.</span></div>
